...
import { useNetwork, MOVE_SEND_INTERVAL_MS, WALL_DAMAGE_SEND_INTERVAL_MS } from "../network/NetworkContext.jsx";
import {
  SPEED,
  CAPSULE_RADIUS,
  CAPSULE_HALF_HEIGHT,
...
export default function Player({
  ...
  const { sendMove, sendWallDamage } = useNetwork();
  const lastMoveSendMs = useRef(0);
  const lastWallSendMs = useRef(0);
  ...
  // Height-normalise the model to MODEL_TARGET_HEIGHT (src/playerModel.js) --
  // shared with RemotePlayer.jsx, which fits its own cloned model the same way.
    ...
    (ndcX, ndcY, trigger, dt) => {
      ...
      if (trigger && hitWall && dt && wallHealth) {
        ...
        if (entry && entry.hp > 0) {
          ...
          if (nowMs - lastWallSendMs.current >= WALL_DAMAGE_SEND_INTERVAL_MS) {
            lastWallSendMs.current = nowMs;
            sendWallDamage(wallType, entry.hp);
          }
        }
      }
    },
  ...
  useFrame((_, dt) => {
    ...
    // Procedural walk cycle (no clips in the glb; src/playerModel.js). gait
    ...
    // Throttled transform sync for other players (src/network/NetworkContext.jsx).
    // No-ops when disconnected. Reuses values already computed above this frame.
    const nowMs2 = performance.now();
    if (nowMs2 - lastMoveSendMs.current >= MOVE_SEND_INTERVAL_MS) {
      lastMoveSendMs.current = nowMs2;
      sendMove({
        x: t.x,
        y: t.y,
        z: t.z,
        yaw: modelYaw.current,
        speed: gait.current,
        firing: controls.firing,
        beamToX: beamState.current.to.x,
        beamToY: beamState.current.to.y,
        beamToZ: beamState.current.to.z,
      });
    }
  });
